...
import org.kwrench.random.XorShift
import java.util.*
...
class WeightedMap<T>(initialEntries: Map<T, Double>? = null): RandomEntries<T> {
    ...
    private val defaultRandom = XorShift()
    ...
    fun addEntry(entry: T, relativeAdditionalWeight: Double) {
        ...
        val weight = Math.max(0.0, relativeAdditionalWeight + (entries[entry] ?: 0.0))
    ...
    override fun nextRandomEntry(rand: Rand): T = randomEntry(defaultRandom)
