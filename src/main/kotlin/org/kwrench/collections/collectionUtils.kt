...
/**
 * @return a randomly picked element from this list that fulfills the specified condition, or null if no such element was found from the collection.
 */
// TODO: This will prefer elements that have a lot of non-matching elements in front of them.  Fix by using a permutation to iterate the list?
fun <T>List<T>.randomElementWith(condition: (T) -> Boolean, random: Rand = Rand.default): T? {
    if (this.isEmpty()) return null
    else {
        val startIndex = random.nextInt(this.size)

        // Return if matching
        if (condition(this[startIndex])) return this[startIndex]

        var index = (startIndex + 1) % this.size
        while (index != startIndex) {
            ...
            // Move to next item, roll over at end of list
