package org.kwrench.math

import org.kwrench.checking.Check
import org.kwrench.math.MathUtils.bigDecimalRoundingMathContext
 ...
 * @param smooth if true, will use cosine interpolation between the start and end values (also automatically clamps the result).  Defaults to false.
...
fun mix(t: Float,
        ...
        clamp: Boolean = false,
        smooth: Boolean = false): Float {
    return if (smooth) start + interpolateCosine(t) * (end - start)
    else {
        // Apply clamping if wanted
        val v = if (clamp) t.clamp0To1() else t
...
fun Float.mixTo(start: Float = 0f,
                ...
                clamp: Boolean = false,
                smooth: Boolean = false): Float {
    return mix(this, start, end, clamp, smooth)
}
...
/**
 * Uses cosine interpolation to interpolate between 0 and 1 as the parameter goes from 0 to 1.
 * Clamps the result to the 0..1 interval.
 * @return smoothly interpolated value
 */
fun interpolateCosine(t: Float): Float {
    return when {
        t <= 0f -> 0f
        t >= 1f -> 1f
        else -> (0.5 - Math.cos(t * Math.PI) * 0.5).toFloat()
    }
}
 ...
 * @return smoothly interpolated value
 */
fun interpolateCosine(t: Double): Double {
    return when {
        t <= 0.0 -> 0.0
        t >= 1.0 -> 1.0
        else -> 0.5 - Math.cos(t * Math.PI) * 0.5
    }
}
...
fun map(t: Float,
    ...
    return mix(r, targetStart, targetEnd, clamp, smooth)
}
...
fun Float.mapTo(sourceStart: Float = 0f,
                ...
                clamp: Boolean = false,
                smooth: Boolean = false): Float {
    return map(this, sourceStart, sourceEnd, targetStart, targetEnd, clamp, smooth)
}
