 ...
 * @param block code to run
 */
inline fun <T>ignoringExceptions(ignoredExceptions: List<KClass<out Throwable>>, block: () -> T?): T? {
 ...
 * @param block code to run
 */
inline fun <T>ignoringExceptions(ignoredExceptions: List<KClass<out Throwable>>, resultOnFail: T, block: () -> T): T {
    return try {
        block()
    }
    catch (e: Throwable) {
