...
public class BitVector {
    ...
    /** Creates a bit set based off a collection of integers (each bit corresponding to a provided integer will be set).
     * @param integers */
    public BitVector(Collection<Integer> integers) {
        set(integers);
    }
    ...
    /**
     * Set this BitVector to the same value as the other bit vector.
     */
    public void set(BitVector other) {
        if (words.length == other.words.length) {
            // Copy data directly if same size
            System.arraycopy(other.words, 0, words, 0, words.length);
        }
        else {
            // Create new array if different size
            words = Arrays.copyOf(other.words, other.words.length);
        }
    }

    /**
     * Set the bits corresponding to the integers provided and clear all other bits.
     * @param integers the bits to set
     */
    public void set(Collection<Integer> integers) {
        clear();

        // Ensure the capacity is correctly set at the start, to avoid unnecessary array creation and copying
        int largest = 0;
        for (Integer i : integers) {
            if (i > largest) largest = i;
        }
        ensureCapacity(largest);
