...
/*
 * SuperSimplexNoise by KdotJPG
 * https://www.reddit.com/r/proceduralgeneration/comments/68kpwt/most_important_algorithms_to_know/dgzr47x/
 *
 * Changed random to custom random class.
 */


import org.kwrench.random.Rand;
import org.kwrench.noise.NoiseValueSchemes.*;
...
public class SuperSimplexNoise {
    ...
    public SuperSimplexNoise(Rand random) {
        perm = new short[1024];
        perm2D = new short[1024];
        perm3D = new short[1024];
        short[] source = new short[1024];
    ...
    public void eval_D2C1(double[] destination, int destinationIndex, double x, double y) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            d[di+ NoiseValueSchemes.D2C1.F] += attnSq * attnSq * extrapolation;
            d[di+ NoiseValueSchemes.D2C1.Fx] += (gx * attn - 8 * dx * extrapolation) * attnSq * attn;
            d[di+ NoiseValueSchemes.D2C1.Fy] += (gy * attn - 8 * dy * extrapolation) * attnSq * attn;
            d[di+ NoiseValueSchemes.D2C1.Fxy] += 8 * attnSq * (6 * extrapolation * dx * dy - attn * (gx * dy + gy * dx));
        }
    }
    ...
    public static class Octaves {
        ...
        public Octaves(Rand random, int octaves) {
    ...
    public static class Octaves_Multi {
        ...
        public final double max;
        public Octaves_Multi(Rand random, int octaves, int instances) {
